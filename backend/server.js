...
app.post("/api/user/register", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "register successfully", token, user });
  } catch (e) {
    res
      .status(500)
...
const isAbhishek = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(201)
        .json({ success: false, message: "User Not found with the token" });
    }
    if (user.email !== "abhishekmishra.dev01@gmail.com") {
      return res.json({ success: false, message: "You are not Abhishek" });
    }
    req.user = user;
    next();
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: `isAbhishek mdlware error ${error.message}`,
    });
  }
};
...
app.get("/get-projects", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, mesaage: "Couldn't get the projects" });
  }
});

app.delete("/projects/:id", isAbhishek, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res
      .status(500)
      .json({ success: false, message: "Couldn't delete the project" });
  }
});
